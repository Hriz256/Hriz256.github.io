...
AEFTBridge = {
	...
	placeDocument: function(pathToFile) {
		app.beginUndoGroup('Import File and Place in New Composition');

		var theProject = app.project;
		var alertString = 'Please select a valid footage file.';

		// if a path has been passed to the function
		if (pathToFile) {
			// Avoid passing in a folder
			var folderCheck = Folder(pathToFile);

			if (folderCheck.constructor == Folder && folderCheck.exists) {
				alert(alertString);
				app.endUndoGroup();
				return;
			}

			var fileAtPath = File(pathToFile);

			// check if the path is valid
			if (fileAtPath.exists) {
				var fileToImport = new ImportOptions(fileAtPath);
			} else {
				alert(alertString);
				app.endUndoGroup();
				return;
			...
			var fileToImport = new ImportOptions(File(pathToFile));

			// Check if file can be imported as footage
		...
		// import the file, via valid path string or import dialog
		var theFile = pathToFile
			? theProject.importFile(fileToImport)
			: theProject.importFileWithDialog();

		// if Cancel was clicked on the import dialog
		if (!theFile) {
			alert(alertString);
			app.endUndoGroup();
			return;
		...
		// define file extension and name as strings
		var fileType = pathToFile
			? theFile.name.split('.').pop()
			: theFile[0].name.split('.').pop();
		var fileName = pathToFile
			? theFile.name.split('.').slice(0, -1)
			: theFile[0].name.split('.').slice(0, -1);

		// if the imported footage is .jsx file, alert the user and remove it to avoid a crash.
		if (fileType == 'jsx') {
			app.project.selection[0].remove();
			alert(alertString);
			app.endUndoGroup();
			return;
		}

		app.project.showWindow(false); // toggle the project panel
		app.project.showWindow(true);

		app.executeCommand(2796); // create new composition from selection

		// Since we executed an app command, the new comp wasn't immediately returned.
		// Loop through project items looking for a comp matching the import file's name.
		for (var i = 1, il = theProject.numItems; i <= il; i++) {
			var item = theProject.items[i];

			if (item instanceof CompItem && item.name == fileName) {
				app.endUndoGroup();
				return true;
			}
		}
			...
			if (welcomeDesc !== undefined) {
				if (welcomeDesc.count) {
					...
					w.sessionGUID = welcomeDesc
						...
						.replace(/-/g, '');
					try {
						// Changes for older version, not required after NGL.
						w.appStartClockTime = welcomeDesc.getLargeInteger(
							kstartTimeStr
						);
						w.firstName = welcomeDesc.getString(kfirstStr);
						w.lastName = welcomeDesc.getString(klastStr);
						w.accountStatus =
							welcomeDesc.getInteger(kentryStatusStr) === 1
								? 'trial'
								: 'paid';
						w.accountType = welcomeDesc.getString(kaccountTypeStr);
						w.secondsLeftInTrial =
							w.accountStatus === 'paid'
								? 0
								: welcomeDesc.getLargeInteger(kleftStr);
						w.countryCode = welcomeDesc.getString(
							kisoCountryCodeStr
						);
					} catch (ignore) {}
	...
	getCCXUserJSON: function(mode, knownList) {
		...
		if (isFNFT) {
			...
			try {
				...
				supportJSONBlocks.homescreenVisibilityEventId = app.stringIDToTypeID(
