...
$._ADBE_LIBS_CORE = {
    ...
    chooseFileLocation: function (promptStr, path, libraryName) {
        ...
        var file = new File(path === "null" ? Folder.desktop.toString() + separator + sanitizedLibraryName + ".cclibs" : path);
        var result = file.saveDlg(promptStr, '*.cclibs', false);
    ...
    chooseFile: function (promptStr, path) {
        ...
        var filter = null;
        if ($.os.indexOf('Windows') > -1) {
            filter = "Creative Cloud Libraries: *.cclibs";
        } else {
            var filterRegex = new RegExp('[^\.]*\.cclibs$');
